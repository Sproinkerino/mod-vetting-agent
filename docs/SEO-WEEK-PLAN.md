...
Mobile visual QA (19 September 2026): Chrome device emulation exposed a minimum-content grid overflow that expanded a 390px page to 481px. The local fix makes the home grid and hero shrink to the device viewport while leaving the subreddit chip row independently scrollable. Automated measurements pass at 320px, 360px, and 390px with no document-level horizontal overflow; a visual 390px pass and a new regression test also pass. Deployment and production remeasurement remain the next release gate.
