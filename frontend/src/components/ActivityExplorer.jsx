...
export default function ActivityExplorer({ activity = [] }) {
  ...
  return <section className='activity-section'>
    <div className='activity-heading'><div><span className='eyebrow'>Source archive</span><h2>Fetched public activity</h2><p>Every item in the analysis window, not only flagged evidence.</p></div><input aria-label='Search activity' value={query} onChange={(e)=>search(e.target.value)} placeholder='Search activity' /></div>
    <div className='activity-tabs' role='tablist'>{['comment','post'].map((kind)=><button key={kind} role='tab' aria-selected={type===kind} className={type===kind?'active':''} onClick={()=>changeType(kind)}>{kind==='comment'?'Comments':'Posts'} <span>{activity.filter((item)=>item.type===kind).length}</span></button>)}</div>
    <div className='activity-list'>{visible.map((item)=><article className='activity-row' key={`${item.type}-${item.id}`}><div className='activity-meta'><span>r/{item.subreddit}</span><span>{fmtDate(item.created_utc)}</span><span>{item.score} points</span></div><h3>{item.type==='post'?(item.title||'Untitled post'):(item.submission_title||'Reddit discussion')}</h3><p>{item.body||'No text content.'}</p><a href={item.permalink} target='_blank' rel='noreferrer'>Open source ↗</a></article>)}{!visible.length&&<div className='empty-panel'>No matching {type}s.</div>}</div>
