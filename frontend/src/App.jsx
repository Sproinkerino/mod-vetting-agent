...
function AskPanel({ report }) {
  ...
  async function ask(event) { event.preventDefault(); if (!question.trim()) return; setBusy(true); setError(''); try { setResult(await askArchive(report.api_job_id, question.trim(), report, sourceCount)); } catch (reason) { setError(reason.message); } finally { setBusy(false); } }
